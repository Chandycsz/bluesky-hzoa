...
public interface AccountMapper {
    @Insert("insert into account " +
            "(name, nick_name,head_img, mobile,role_type, role_id, organization_id, department, id_card, password, last_login_ip, last_login_time, authorities_text, sort, status, created_at, updated_at) " +
            "values " +
            "(#{account.name},#{account.nickName},#{account.headImg},#{account.mobile},#{account.roleType},#{account.roleId},#{account.organizationId},#{account.department},#{account.idCard},#{account.password},#{account.lastLoginIp},#{account.lastLoginTime},#{account.authoritiesText},#{account.sort},#{account.status},now(),now())")
